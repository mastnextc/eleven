...
import { ShoppingBag, TrendingUp, Users, Package, Plus, Trash2, LogOut } from 'lucide-react';
import Logo from '../components/Logo';

const Dashboard = ({ token, user, onLogout, apiUrl }) => {
  ...
  const fetchAdminData = async () => {
    try {
      ...
      const resStats = await fetch(`${apiUrl}/admin/dashboard`, {
      ...
      if (resStats.ok) {
        ...
        setAllOrders(stats.latestOrders); // Populate order manager
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      {/* Top Navbar */}
      <header className="bg-white border-b border-stone-100 px-6 py-4 flex justify-between items-center shadow-sm">
        <Logo className="h-9 w-auto" />
        <div className="flex items-center space-x-4">
          <span className="text-xs font-semibold text-stone-600">Signed in as: <strong className="text-stone-900">{user?.name}</strong></span>
          <button
            onClick={onLogout}
            className="flex items-center space-x-1.5 text-stone-500 hover:text-stone-900 transition-colors text-xs font-bold uppercase tracking-wider"
          >
                ...
                <div className="space-y-8 animate-fadeIn">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
                    <div className="bg-white border p-5 rounded-2xl shadow-sm flex items-center space-x-4">
                      <div className="bg-stone-50 text-stone-900 p-3 rounded-full"><TrendingUp className="w-5 h-5" /></div>
                      <div>
                    ...
                    <div className="bg-white border p-5 rounded-2xl shadow-sm flex items-center space-x-4">
                      <div className="bg-stone-50 text-stone-900 p-3 rounded-full"><ShoppingBag className="w-5 h-5" /></div>
                      <div>
                    ...
                    <div className="bg-white border p-5 rounded-2xl shadow-sm flex items-center space-x-4">
                      <div className="bg-stone-50 text-stone-900 p-3 rounded-full"><Users className="w-5 h-5" /></div>
                      <div>
                    ...
                    <div className="bg-white border p-5 rounded-2xl shadow-sm flex items-center space-x-4">
                      <div className="bg-stone-50 text-stone-900 p-3 rounded-full"><Package className="w-5 h-5" /></div>
                      <div>
                      ...
                      <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="text-xs divide-y divide-stone-50">
                          {latestOrders.map((ord) => (
                            <tr key={ord.id || ord._id} className="hover:bg-stone-50/50">
                              <td className="px-6 py-3.5 font-semibold text-stone-900 truncate max-w-[120px]">{ord.id || ord._id}</td>
                ...
                <div className="space-y-4 animate-fadeIn">
                  ...
                  <div className="bg-white border border-stone-100 rounded-2xl overflow-hidden shadow-sm">
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="text-xs divide-y divide-stone-50">
                        {products.map((prod) => (
                          <tr key={prod.id || prod._id} className="hover:bg-stone-50/50">
                            <td className="px-6 py-3.5">
